#!/usr/bin/env python3
"""
Working Plati Sales Tracker
Following the corrected script from user
"""
...
def fetch_sales_v2(token):
    ...
    print(f"\n📊 Fetching sales from {start_date} to {end_date}...")
...
def save_report(data):
    ...
    with open(LOG_FILE, "w") as f:
        ...
        if not rows:
            print("⚠️  No sales found in this period.")
            ...
            for item in rows:
                ...
                amount = float(item.get("amount_in", 0))
            ...
            f.write(f"\n---\n")
            f.write(f"Total Sales: {count}\n")
            f.write(f"Total Revenue: {total_revenue} RUB\n")
            f.write(f"Estimated USD: ${total_revenue/95:.2f}\n")
            
            print(f"\n✅ Success! Found {count} sales.")
            print(f"💰 Total Revenue: {total_revenue} RUB (~${total_revenue/95:.2f} USD)")
            print(f"📝 Saved to {LOG_FILE}")

if __name__ == "__main__":
    print("=" * 60)
    print("🚀 Plati Sales Tracker (Working Version)")
    print("=" * 60)
    ...
    if tk:
        ...
        print("\n❌ Could not authenticate. Check API key.")
